...
async function exchangeApduOverHid(apdu: Buffer): Promise<Buffer> {
  const device = await openLedgerDevice();
  try {
 ...
 *
 * BLOCKING (apart from the lazy `node-hid` import in `openLedgerDevice`):
 * `node-hid`'s underlying write/read calls genuinely block the calling
 * thread for the duration of the device I/O — including however long the
 * user takes to physically confirm on-device. This must only ever be
 * called from within the worker thread `worker.ts` spawns, never directly
 * on a server's main thread (see index.ts's `signHederaPayload`, which is
 * the non-blocking entry point everything outside this package should
 * use).
 *
 ...
 */
export async function signHederaPayloadOverHid(rawTransactionBody: Buffer, keyIndex = 0): Promise<Buffer> {
  const apdu = buildSignTransactionApdu(keyIndex, rawTransactionBody);
  const response = await exchangeApduOverHid(apdu);
  return parseSignTransactionResponse(response);
}

/**
 * Signs `rawTransactionBody` the same way as `signHederaPayloadOverHid`, but
